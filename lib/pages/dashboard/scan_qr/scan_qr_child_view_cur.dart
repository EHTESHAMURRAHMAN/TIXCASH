...
class ScanQRChildCur extends GetView<ScanQrController> {
  final bool hasActionBar;
  final CurrencyResponse? currencyResponse;
  const ScanQRChildCur({
    Key? key,
  ...
  Widget build(BuildContext context) {
    // Get.isRegistered<ScanQrController>() ? Get.find<ScanQrController>() : Get.put(ScanQrController());

    Get.isRegistered<ScanQrController>()
        ? Get.find<ScanQrController>()
        : Get.put(ScanQrController());

    controller.getAddressId(currencyResponse?.currency);
    ...
    controller.screenshotController.capture().then((Uint8List? image) async {
      if (image != null) {
        try {
          ...
          Share.shareFiles([imagePath.path], text: controller.address.value);
        } catch (error) {}
      }
    }).catchError((onError) {
      print('Error --->> $onError');
